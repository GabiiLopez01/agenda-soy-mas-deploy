...
function Compromiso ({ tipo = 'DEPORTE', descripcion = 'Yoga 30 min', icon = null, onChecksChange }){
  ...
  const toggle = (i) => {
    const copy = [...checked];
    // oldValue no se usa aquí, pero sería útil para:
    // - Validar cambios: if (oldValue) return;
    // - Logging: console.log(oldValue ? 'Desmarcó' : 'Marcó');
    // - Rollback en errores: catch { copy[i] = oldValue; }
